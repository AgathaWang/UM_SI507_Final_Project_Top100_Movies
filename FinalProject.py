...
valid_total_list = valid_command_list + valid_orderby_list + valid_tb_list + valid_plot_list

def formatted_input(word_list):
    formatted_input_dict = {'command':'movie','orderby':'year','tb':'top','limit':10,'figure':'barplot'}
...
def process_command(command):  # command is a string
    ...
    result = ''
    for word in word_list:
        if (not word.isnumeric()) and (not word in valid_total_list):
            result = 'error'
            return result
